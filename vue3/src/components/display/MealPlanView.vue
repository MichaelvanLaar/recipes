    ...
    <v-col class="pb-0">
      <v-card class="h-100" :loading="useMealPlanStore().loading">
        ...
        <calendar-view
          ...
          @click-date="
            (date: Date, calendarItems: [], windowEvent: any) => {
              newPlanDialogDefaultItem.fromDate = date
              newPlanDialogDefaultItem.toDate = date
              newPlanDialog = true
            }
          "
        >
          ...
          <template #item="{ value, weekStartDate, top }">
            <meal-plan-calendar-item
              ...
              @delete="
                (arg: MealPlan) => {
                  useMealPlanStore().plans.delete(arg.id)
                }
              "
      ...
      <model-edit-dialog
        ...
        :itemDefaults="newPlanDialogDefaultItem"
        :close-after-create="false"
        @create="(arg: any) => useMealPlanStore().plans.set(arg.id, arg)"
      ></model-edit-dialog>
    </v-col>
  </v-row>
</template>
...
function refreshVisiblePeriod(startDateUnknown: boolean) {
  ...
  if (startDateUnknown) {
    useMealPlanStore().refreshFromAPI(
      DateTime.fromJSDate(calendarDate.value).minus({ days: days }).toJSDate(),
      DateTime.fromJSDate(calendarDate.value).plus({ days: days }).toJSDate()
    )
  } else {
...
function dropCalendarItemOnDate(undefinedItem: IMealPlanNormalizedCalendarItem, targetDate: Date, event: DragEvent) {
  ...
  if (currentlyDraggedMealplan.value.originalItem.mealPlan.id != undefined) {
    ...
    if (mealPlan != undefined) {
      ...
      const newFrom = DateTime.fromJSDate(targetDate)
        .set({
        ...
        .toJSDate()
      const newTo = DateTime.fromJSDate(targetDate)
        .plus(fromToDiff)
        .set({
          hour: existingTime.hour,
          minute: existingTime.minute,
