...
for dotfile in .gitignore .npmignore .prettierignore .editorconfig .nvmrc .node-version .vale.ini .markdownlint.json .markdownlint.yaml .markdownlint.yml .prettierrc .claudeignore; do
  [[ -f "$ROOT/$dotfile" ]] && config_files+=("$dotfile")
done

# Root-level named config files
for named in pytest.ini; do
  [[ -f "$ROOT/$named" ]] && config_files+=("$named")
done

# .claude/ direct children (skip subdirectories)
...
while IFS= read -r line; do
  ...
  if $section_found; then
    if [[ "$line" =~ ^\|[[:space:]]*\`([^\`]+)\`[[:space:]]*\|[[:space:]]*(.+)[[:space:]]*\| ]]; then
      ...
      descriptions["$file"]="${descriptions[$file]%"${descriptions[$file]##*[![:space:]]}"}"
    fi
  fi
done < "$CLAUDE_MD"
